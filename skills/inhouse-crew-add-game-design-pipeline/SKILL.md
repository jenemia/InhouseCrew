...
7. Update `tests/test_persona_loader.py` and `tests/test_crew_factory.py` whenever the sample registry or crew catalog changes.
8. Keep `README.md` to exactly one persona example block. Update sample crew mentions without adding more persona examples.
9. Update `AGENTS.md` so future sessions know when to use this skill.
10. Validate the new skill and run the relevant pytest targets after config changes.

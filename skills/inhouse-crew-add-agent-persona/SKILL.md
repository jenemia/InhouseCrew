...
2. Add or update exactly one persona file under `configs/agents/` unless the user explicitly asks for a batch change.
3. Keep persona ids in `snake_case` and compatible with `AgentPersona` in `src/inhouse_crew/persona_loader.py`.
4. Update `tests/test_persona_loader.py` whenever the set of sample agents changes.
5. Keep `README.md` to exactly one persona file example. Replace the existing example instead of adding another.
6. Do not modify `configs/crews/*.yaml` unless the user explicitly asks for crew changes.
7. If the request is implementation work, create or update a `docs/*-implementation.md` spec before editing production files.
8. Run `uv run pytest tests/test_persona_loader.py` after changing persona configs or registry expectations.
